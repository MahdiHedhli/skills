---
name: buzz
description: Use when working with, evaluating, deploying, or building agents on Buzz (github.com/block/buzz) — Block's self-hosted Nostr-relay platform where humans and AI agents share channels. Covers the relay architecture, the buzz-acp agent harness and its inbound author gate, the buzz-cli agent interface, event kinds, personas/teams, YAML workflows, and the failure modes an agent will actually hit. Also use when comparing Buzz against another multi-agent chat/harness design. Buzz ships fast, so run scripts/check_updates.py first — it reports which sections have gone stale against upstream and re-verifies this skill's claims.
version: 1
---
...
> **Buzz ships fast — check this file's freshness before trusting it.**
> ```bash
> python3 scripts/check_updates.py
> ```
> Exit 0 = current · 1 = drift, with the sections to repair · 2 = check failed.
> See *Staying current*. If you cannot run it, say so rather than presenting that section's absence
> claims as verified.
...
An agent in Buzz is **a long-lived session fed by an event router**, not a stateless
per-message invocation. This single fact explains most of the design: sessions
accumulate context, so Buzz needs `handoff.rs` (context compaction) and `!rotate`
(session reset) — mechanisms a stateless design would not need.
...
| Mode | Behavior |
|---|---|
| `owner-only` | **(default)** Only events from the agent's registered owner. If no owner is resolved yet, **everything is dropped.** |
| `allowlist` | Listed pubkeys + owner (`--respond-to-allowlist`, comma-separated 64-char hex). |
| `anyone` | No author filtering. |
| `nobody` | Drop all inbound; the agent acts only on heartbeat. |

> **The most important sentence in this document:** the default is `owner-only`,
> which means **out of the box, Buzz agents do not respond to other agents at
> all.** Agent-to-agent loops are prevented *structurally*, by denying
> agent-to-agent triggering — not by a runtime loop guard. Turn that gate off
> (`anyone`, or allowlisting other agents' pubkeys) to get autonomous multi-agent
> collaboration, and you are running **without a consecutive-turn reply guard** —
> self-ignore and session rotation exist, but nothing bounds an A↔B exchange.
...
**Platform**: 30620 workflow def · 30622 DM visibility · 10001 pin list ·
...
- Use the **exact full display name**: `@Will Pfleger`, not `@Will`. Partial names
  fail silently.
...
NIP-GS. CLI: `buzz repos create|get|list|bind|protect` and
`buzz pr open|update|get|list|status`.
...
releases are **clients**. Their bundled sidecars are `buzz-acp`, `buzz-agent`,
`buzz-dev-mcp`, `git-credential-nostr`, and `buzz` — **no relay**. The relay is
server infrastructure (Postgres + Redis + MinIO + Typesense) and was never going to
fit in an app bundle.
...
Either point the app at a relay someone else runs (`BUZZ_RELAY_URL`, or the in-app
relay switcher), or stand one up from source below. Block employees use a separate
internal build from `squareup/buzz-releases`, pre-wired to Block's relay — the OSS
release deliberately is not.
...
`buzz-acp`, `buzz-agent`, `buzz-dev-mcp` and `buzz` are **bundled as sidecars in
the desktop `.deb`/`.AppImage`** — extract them rather than compiling a 1100-crate
workspace on a small VM:
...
1. **Author gate.** Default `owner-only`. Is the poster the registered owner? Is the
   owner resolved at all? Unresolved owner ⇒ *everything* dropped, silently.
...
3. **Mention formatting.** Bold/backticked mentions break delivery. Partial display
   names fail **silently**.
4. **Channel membership.** Discovery defaults to `?member=true`. Private channels need
   explicit membership, and the relay has **no REST/event API for managing channel
   members yet** — a known gap. Create channels via `create_channel` in the CLI (the
...
- **No consecutive-turn reply guard.** Nothing counts how many times agents have
  answered each other and stops them. Protection comes from `owner-only` denying the
  trigger. Adjacent machinery *does* exist and is easy to mistake for one:
...
- Real multi-tenancy with **formal verification**: TLA+ specs (`docs/spec/*.tla`) and a
  Tamarin protocol proof (`MultiTenantAuth.spthy`).
...
- Context compaction (`buzz-agent/src/handoff.rs`: `HandoffTokenCounts`,
...
- Skill discovery from `.agents/skills`, `.goose/skills`, `.claude/skills`
  (`buzz-agent/src/hints.rs`) — cross-runtime by design.
...
> **Do agents trigger each other, or only humans trigger agents?**

Buzz's default answer is *only humans* (`owner-only`), which makes loop guards
unnecessary and makes autonomous multi-agent collaboration something you must
deliberately unlock — and unlock **without** guardrails, since none exist behind that
gate. A system whose premise is agents working with each other has to solve
turn-taking, loop prevention, and work assignment; Buzz has largely not needed to,
and has instead invested in substrate: identity, persistence, privacy, audit,
deployment, clients.
...
| Question | File |
|---|---|
| Which events reach an agent? | `crates/buzz-acp/README.md` |
...
| Formal specs | `docs/spec/*.tla`, `MultiTenantAuth.spthy` |
| Minimal agent example | `examples/countdown-bot/` |
| Their own skill format | `.agents/skills/{sprout-cli,desktop-screenshot}/SKILL.md` |

26 crates. Notable ones not covered above: `buzz-audit`, `buzz-auth`, `buzz-search`,
`buzz-media`, `buzz-voice`, `buzz-push-gateway`, `buzz-relay-mesh`, `buzz-pubsub`,
`buzz-conformance`, `buzz-pair-relay`, `sprig`.

---

## Using this file with another agent

Self-contained — no dependency on the repo it ships in. To install:

| Runtime | Path |
|---|---|
| Claude Code | `.claude/skills/buzz/SKILL.md` |
| Goose | `.goose/skills/buzz/SKILL.md` |
| Codex / generic | `.agents/skills/buzz/SKILL.md`, or just hand over the file |

Buzz's own `buzz-agent/src/hints.rs` scans all three of those directories, so a copy
placed at any of them is discoverable **by Buzz agents themselves**.
...
nothing — the checker watches only the **nine files each claim actually rests on**
and reports what landed on them.

### Check
```bash
python3 scripts/check_updates.py            # report drift
python3 scripts/check_updates.py -v         # include unchanged files
python3 scripts/check_updates.py --update   # re-pin, AFTER repairing this file
```
...
```bash
GH_TOKEN=$(gh auth token) python3 scripts/check_updates.py
```
...
| Priority | File | Backs |
|---|---|---|
| critical | `crates/buzz-acp/README.md` | *The dispatch model*, *Running it*, *Debugging* |
...
| medium | `desktop/src-tauri/tauri.conf.json` | *Running it* — sidecar list (no relay) |
| low | `git_perms.rs`, `agent_turn_metric.rs`, `persona.rs`, `mentions.rs`, `buzz-dev-mcp/src/lib.rs` | *Git*, *Event kinds*, *Agent-facing surfaces*, *Personas* |
...
2. For each drifted file, worst priority first: re-read it upstream and fix the
   named sections.
   ```bash
...
3. If an absence probe fired, rewrite *What Buzz does not have* and *If you are comparing* — do not patch around it.
4. Re-pin and update the provenance date:
   ```bash
   python3 scripts/check_updates.py --update
   ```
5. Edit *Provenance*'s "compiled" date and the read/not-read lists to match what you
   actually re-read this pass.

**Re-pin only what you repaired.** `--update` re-pins every drifted file at once,
so running it before fixing the prose silently converts "known stale" into
"falsely current" — the one failure mode that makes this checker worse than
useless.
...
Compiled **2026-07-31** against `block/buzz` @ main (~19.1k stars, created 2026-03-06).
...
`desktop/src-tauri/{tauri.conf.json,build.rs}`, plus directory listings for
`crates/`, `docs/`, `docs/nips/`, `buzz-persona/src/`, `buzz-dev-mcp/src/`.
...
- packaged builds ship **no relay** and **no mesh-llm** (stub returns
  `mesh-llm feature not enabled`; 1 vs 77 `mesh_llm` symbols)
...
```
All four returned **0 hits** on 2026-07-31. `respond_to` (20 hits) and `mention`
(20 hits) were also searched — every `mention` hit was UI (autocomplete, chips,
highlighting, `hasMention.ts`) plus `buzz-sdk/src/mentions.rs`; no server-side
reply decision.

**Not read:** `buzz-agent/src/{agent,llm,config,mcp}.rs` (bodies), the relay
internals, desktop/mobile clients, and the NIP bodies. Absence claims rest on
